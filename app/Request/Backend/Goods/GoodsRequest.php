    ...
    public function rules(string $ruleKey = ''): array
    {
        ...
        $rules = [
            'post:create' => [
                ...
                'type' => 'required|required|in:' . $this->getStateRuleIn($map['type']),
                'virtual_sales' => 'integer',
                'status' => 'required|integer|in:' . $this->getStateRuleIn($map['status']),
                'recommend_way' => 'required|integer|in:' . $this->getStateRuleIn($map['recommend_way']),
                'is_consume_discount' => 'integer|in:' . $this->getStateRuleIn($map['is_consume_discount']),
                'is_free_shipping' => 'integer|in:' . $this->getStateRuleIn($map['is_free_shipping']),
                'buy_max' => 'integer',
                'buy_min' => 'integer',
                'refund_type' => 'required|string|in:' . $this->getStateRuleIn($map['refund_type']),
            ...
            'post:update' => [
                ...
                'status' => 'integer|in:' . $this->getStateRuleIn($map['type']),
            ],
            'post:updateStatus' => [
                'id' => 'required|integer|gt:0',
                'status' => 'integer|in:' . $this->getStateRuleIn($map['type']),
